...
// 函数简介     初始化Flash句柄
// 参数说明     handle      Flash句柄
// 参数说明     sector_num  扇区编号（仅可填写0）
// 参数说明     page_num    页编号（0-11）
// 参数说明     buf         数据缓冲区
// 参数说明     len         数据长度
// 返回参数     void
// 使用示例     flash_handle_init(&flash_handle, 0, 1, data_buffer, 10);
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
void flash_handle_init(flash_handle_t *handle, uint32 sector_num, uint32 page_num, uint32 *buf, uint16 len)
{
    handle->sector_num = sector_num;
    handle->page_num = page_num;
    handle->data_buffer = buf;
    handle->data_length = len;
    handle->erase_flag = 0;
}
...
// 参数说明     handle      Flash句柄
// 返回参数     void
// 使用示例     flash_handle_read(&flash_handle);
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
void flash_handle_read(flash_handle_t *handle)
{
    flash_read_page(handle->sector_num, handle->page_num, handle->data_buffer, handle->data_length);
}
...
// 参数说明     handle      Flash句柄
// 返回参数     void
// 使用示例     flash_handle_write(&flash_handle);
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
void flash_handle_write(flash_handle_t *handle)
{
    if(handle->erase_flag)
    {
        flash_erase_page(handle->sector_num, handle->page_num);
        handle->erase_flag = 0;
    }
    flash_write_page(handle->sector_num, handle->page_num, handle->data_buffer, handle->data_length);
}
...
// 参数说明     handle      Flash句柄
// 返回参数     void
// 使用示例     flash_handle_erase(&flash_handle);
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
void flash_handle_erase(flash_handle_t *handle)
{
    flash_erase_page(handle->sector_num, handle->page_num);
    handle->erase_flag = 1;
}
...
// 函数简介     检查Flash页是否有数据
// 参数说明     handle      Flash句柄
// 返回参数     uint8       1-有数据，0-无数据
// 使用示例     flash_handle_check(&flash_handle);
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
uint8 flash_handle_check(flash_handle_t *handle)
{
